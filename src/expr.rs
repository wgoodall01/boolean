...
fn ex_and(a: Expr, b: Expr) -> Expr {
    Box::new(ExprValue::And(a, b))
}

fn ex_or(a: Expr, b: Expr) -> Expr {
    Box::new(ExprValue::Or(a, b))
}

fn ex_not(a: Expr) -> Expr {
    Box::new(ExprValue::Not(a))
}

fn ex_implies(a: Expr, b: Expr) -> Expr {
    Box::new(ExprValue::Implies(a, b))
}

fn ex_biconditional(a: Expr, b: Expr) -> Expr {
    Box::new(ExprValue::Biconditional(a, b))
}

fn ex_id(name: &str) -> Expr {
...
fn literal_true<'a>(input: &'a [Token]) -> ParseResult<'a, Expr> {
    map(literal(&Token::True), |_| ex_true()).parse(input)
}

fn literal_false<'a>(input: &'a [Token]) -> ParseResult<'a, Expr> {
    map(literal(&Token::False), |_| ex_false()).parse(input)
}

fn identifier<'a>(input: &'a [Token]) -> ParseResult<'a, Expr> {
    match input.iter().next() {
        Some(Token::Identifier(name)) => Ok((ex_id(name), &input[1..])),
        _ => Err(input),
    }
}
...
fn negation<'a>(input: &'a [Token]) -> ParseResult<'a, Expr> {
    map(right(literal(&Token::Not), expr), |e| ex_not(e)).parse(input)
}
...
mod test {
    ...
    fn test_parse_pair() {
        ...
        assert_eq!(r1, ex_true());
        assert_eq!(r2, ex_false());
    ...
    fn test_parse_atom() {
        ...
        let ((r1, r2), rest) = parser.parse(input).unwrap();
        assert_eq!(r1, ex_false());
        assert_eq!(r2, ex_id("Whatever"));
        assert_eq!(rest.len(), 0);
    }
    ...
    fn test_parse_negation() {
        ...
        assert_eq!(result, ex_not(ex_false()));
        assert_eq!(rest.len(), 0);
    }
    ...
    fn test_parse_and() {
        ...
        assert_eq!(result, ex_and(ex_true(), ex_false()));
        assert_eq!(rest.len(), 0);
    }
    ...
    fn test_parse_and_or() {
        ...
        let (result, rest) = expr(input).unwrap();
        assert_eq!(result, ex_or(ex_id("a"), ex_and(ex_id("b"), ex_id("c"))));
        assert_eq!(rest.len(), 0);
    }
}
